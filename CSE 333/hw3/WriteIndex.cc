...
int WriteIndex(MemIndex* mi, DocTable* dt, const char* file_name) {
  ...
  if (mi_bytes == kFailedWrite) {  // error
    fclose(f);
    unlink(file_name);  // delete the file
    return kFailedWrite;
  }
  ...
  if (WriteHeader(f, dt_bytes, mi_bytes) == kFailedWrite) {  // error
    fclose(f);
    unlink(file_name);  // delete the file
    return kFailedWrite;
  }
...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  // Read and calculate CRC checksum
  for (i = 0; i < doctable_bytes + memidx_bytes; i++) {
    if (fread(&curr, sizeof(uint8_t), 1, f) != 1) {
      return kFailedWrite;
    }
    crc.FoldByteIntoCRC(curr);
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  int record_bytes, bucket_bytes;
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    // Write a BucketRecord for this bucket.
    record_bytes = WriteHTBucketRecord(f, record_pos,
      LinkedList_NumElements(ht->buckets[i]), bucket_pos);
    ...
    if (LinkedList_NumElements(ht->buckets[i]) == 0) {
      continue;

    // Bucket is not empty: Write
    } else {
      bucket_bytes = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    int32_t element_bytes = fn(f, element_pos, element);
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  int16_t file_name_length = strlen(static_cast<const char*>(kv->value));


  // fwrite() the docid from "kv".  Remember to convert to
  // disk format before writing.
  DoctableElementHeader header(kv->key, file_name_length);
  header.ToDiskFormat();
  ...
  if (fwrite(kv->value, file_name_length, 1, f) != 1) {
    return kFailedWrite;
  }

  // STEP 11.
  // Calculate and return the total amount written.
  return sizeof(DoctableElementHeader) + file_name_length;
}
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  int ht_bytes = WriteHashTable(
    f,
    offset + sizeof(WordPostingsHeader) + wordlen,
    wp->postings,
  ...
  WordPostingsHeader header(wordlen, ht_bytes);
  header.ToDiskFormat();
  if (fseek(f, offset, SEEK_SET) != 0) {
    return kFailedWrite;
  }
  ...
  if (fwrite(wp->word, sizeof(char), wordlen, f) != (size_t) wordlen) {
    return kFailedWrite;
  }
  ...
  // STEP 19.
  // Calculate and return the total amount of data written.
  return sizeof(WordPostingsHeader) + wordlen + ht_bytes;
}
}  // namespace hw3
