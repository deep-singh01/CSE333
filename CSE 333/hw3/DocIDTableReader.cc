...
bool DocIDTableReader::LookupDocID(
  ...
  for (IndexFileOffset_t& curr_element : elements) {
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      DocIDElementPosition curr_pos;

      // Read positions and push into list
      for (i = 0; i < curr_header.num_positions; i++) {
        Verify333(fread(&curr_pos, sizeof(DocPositionOffset_t), 1, file_) == 1);
        curr_pos.ToHostFormat();
        positions.push_back(curr_pos.position);
      }
