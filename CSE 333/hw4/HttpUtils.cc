...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // Get the absolute path of the root directory and the test file.
  if ((realpath(root_dir.c_str(), root_path) == nullptr) ||
    (realpath(test_file.c_str(), file_path) == nullptr)) {
    return false;
  }
  ...
  // Ensure that the root path ends with a '/'.
  if (root_path_str.at(root_path_str.length() - 1) != '/') {
    root_path_str.append("/");
  }

  // Make sure that the file path is within the root path and
  // that the file exists.
  if ((file_path_str.find(root_path_str) != 0) ||
    (access(file_path_str.c_str(), F_OK) != 0)) {
    return false;
  }
