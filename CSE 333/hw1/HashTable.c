...
// Checks if a key exists in a chain, and based on what operation is being
// performed, executes the appropriate commands.
bool KeyExists(LLIterator *iter, HTKey_t key, bool delete,
                bool find, HTKeyValue_t *keyvalue);
...
bool HashTable_Insert(HashTable *table,
  ...
  iter = LLIterator_Allocate(chain);
  Verify333(iter != NULL);

  // Key does not exist
  if (!(KeyExists(iter, newkeyvalue.key, false, false, oldkeyvalue))) {
    kv = (HTKeyValue_t *)malloc(sizeof(HTKeyValue_t));
    Verify333(kv != NULL);

    // Set & push the key and value
    *kv = newkeyvalue;
...
bool HashTable_Find(HashTable *table,
  ...
  // Table or chain is empty
  if (table->num_elements == 0 || LinkedList_NumElements(chain) == 0) {
    return false;
  }
  ...
  // Key does not exist
  if (!(KeyExists(iter, key, true, false, keyvalue))) {
    return false;
  }
...
bool HashTable_Remove(HashTable *table,
  ...
  // Key does not exist
  if (!(KeyExists(iter, key, false, true, keyvalue))) {
    return 0;
  }
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  // If table is invalid or empty, or current chain is empty, or
  // current node in current chain is invalid, return false
  return (iter->ht != NULL) && (iter->ht->num_buckets > 0) &&
    (iter->bucket_it != NULL) && LLIterator_IsValid(iter->bucket_it);
}
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // Iterator is valid
    ...
    // Next node in current chain is not null
    if (LLIterator_Next(iter->bucket_it)) {
      return true;
    }

    // Next node in current chain is null
...
bool KeyExists(LLIterator *iter, HTKey_t key, bool find,
                bool delete, HTKeyValue_t *keyvalue) {
  HTKeyValue_t *kv;
